	...
	<tbody>
		...
		<?php foreach($orderpanel->orders as $order) : ?>
			<tr class="<?= $orderpanel->generate_rowclass($order); ?>" id="<?= $order->orderno; ?>">
				...
				<td><?= $order->orderno; ?></td>
				<td><a href="<?php $orderpanel->generate_customerurl($order); ?>"><?= $order->custid; ?></a> <span class="glyphicon glyphicon-share" aria-hidden="true"></span><br><?= get_customername($order->custid); ?></td>
				<td><?= $order->custpo; ?></td>
				<td>
			...
			<?php if ($order->orderno == $input->get->text('ordn')) : ?>
				...
				<?php include $config->paths->content.'dashboard/sales-orders/detail-rows.php'; ?>

				<?php include $config->paths->content.'dashboard/sales-orders/total-rows.php'; ?>

				<?php if ($input->get->text('show') == 'tracking') : ?>
					<?php include $config->paths->content.'dashboard/sales-orders/tracking-rows.php'; ?>
				<?php endif; ?>
