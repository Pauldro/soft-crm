<?php
	...
	$formatjson = json_decode(file_get_contents($config->companyfiles."json/cioifmattbl.json"), true);
	$columns = array_keys($formatjson['data']['detail']);
	$postarray = array();

	if ($input->requestMethod() == "POST") {
		$maxrec = getmaxtableformatterid($user->loginid, 'ci-open-invoices', false);
		...
		if ($input->post->text('action') == 'add-formatter') {
			if (checkformatterifexists($user->loginid, 'ci-open-invoices', false)) {
				...
				$response = addformatter($user->loginid, 'ci-open-invoices', json_encode($json), false);
			...
			$response = editformatter($user->loginid, 'ci-open-invoices', json_encode($json), false);
		...
		if (checkformatterifexists($user->loginid, 'ci-open-invoices', false)) {
			$defaultjson = json_decode(getformatter($user->loginid, 'ci-open-invoices', false), true);
		} else {
			$default = $config->paths->content."item-information/screen-formatters/default/ci-open-invoices.json";
