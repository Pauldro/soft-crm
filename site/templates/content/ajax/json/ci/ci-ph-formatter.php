<?php
	...
	$formatjson = json_decode(file_get_contents($config->companyfiles."json/cipyfmattbl.json"), true);
	$columns = array_keys($formatjson['data']['detail']);
	$postarray = array();

	if ($input->requestMethod() == "POST") {
		$maxrec = getmaxtableformatterid($user->loginid, 'ci-payment-history', false);
		...
		if ($input->post->text('action') == 'add-formatter') {
			if (checkformatterifexists($user->loginid, 'ci-payment-history', false)) {
				...
				$response = addformatter($user->loginid, 'ci-payment-history', json_encode($json), false);
			...
			$response = editformatter($user->loginid, 'ci-payment-history', json_encode($json), false);
		...
		if (checkformatterifexists($user->loginid, 'ci-payment-history', false)) {
			...
			$default = $config->paths->content."item-information/screen-formatters/default/ci-payment-history.json";
