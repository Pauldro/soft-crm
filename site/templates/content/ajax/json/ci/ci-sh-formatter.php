<?php
	...
	$formatjson = json_decode(file_get_contents($config->companyfiles."json/cishfmattbl.json"), true);
	...
	if ($input->requestMethod() == "POST") {
		$maxrec = getmaxtableformatterid($user->loginid, 'ci-sales-history', false);
		...
		if ($input->post->text('action') == 'add-formatter') {
			if (checkformatterifexists($user->loginid, 'ci-sales-history', false)) {
				...
				$response = addformatter($user->loginid, 'ci-sales-history', json_encode($postarray), false);
			...
			$response = editformatter($user->loginid, 'ci-sales-history', json_encode($postarray), false);
		...
		if (checkformatterifexists($user->loginid, 'ci-sales-history', false)) {
			...
			$default = $config->paths->content."cust-information/screen-formatters/default/ci-sales-historys.json";
