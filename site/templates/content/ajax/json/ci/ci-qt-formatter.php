<?php
	...
	$formatjson = json_decode(file_get_contents($config->companyfiles."json/ciqtfmattbl.json"), true);
	...
	if ($input->requestMethod() == "POST") {
		...
		if ($input->post->text('action') == 'add-formatter') {
			if (checkformatterifexists($user->loginid, 'ci-quotes', false)) {
				$json = array (
					'response' => array (
							'error' => true,
							'notifytype' => 'danger',
				...
				$response = addformatter($user->loginid, 'ci-quotes', json_encode($postarray), false);
			...
			$response = editformatter($user->loginid, 'ci-quotes', json_encode($postarray), false);
		...
		if (checkformatterifexists($user->loginid, 'ci-quotes', false)) {
			$defaultjson = json_decode(getformatter($user->loginid, 'ci-quotes', false), true);
		} else {
