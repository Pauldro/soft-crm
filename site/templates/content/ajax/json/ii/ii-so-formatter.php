<?php
	...
	$formatjson = json_decode(file_get_contents($config->companyfiles."json/iisofmattbl.json"), true);
	$columns = array_keys($formatjson['data']['detail']);
	$postarray = array();

	if ($input->requestMethod() == "POST") {
		$maxrec = getmaxtableformatterid($user->loginid, 'ii-sales-order', false);
		...
		if ($input->post->text('action') == 'add-formatter') {
			if (checkformatterifexists($user->loginid, 'ii-sales-order', false)) {
				...
				$response = addformatter($user->loginid, 'ii-sales-order', json_encode($json), false);
			...
			$response = editformatter($user->loginid, 'ii-sales-order', json_encode($json), false);
