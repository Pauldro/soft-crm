...
class ProcessWire extends Wire {
	...
	public static function buildConfig($rootPath, $rootURL = null, array $options = array()) {
		...
		//CPTECH ADDITIONS TO PATHS & URLS

		/* DEPRECATED PATHS
			$config->urls->ajax = "$siteDir/templates/content/ajax/";
			$config->urls->redirectfiles = "$siteDir/templates/redir/";
			$config->urls->functions = $config->urls->templates . 'functions/';
			$config->urls->libraries = "$siteDir/templates/lib/";
		*/

		$config->urls->assetimages = "$assetsDir/files/images/"; //WAS asset_images
		$config->urls->content = "$siteDir/templates/content/";
		$config->urls->css = $config->urls->templates . 'styles/';
		$config->urls->js = $config->urls->templates . 'scripts/';
